# -*- coding: utf-8 -*-
"""Floating Point Representation.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/12nSHKI6ejUtlXsSnH445SCDzfKOPyuXJ
...
We will implement an 8 bit Floating point number system in this part, where First bit is a sign bit, next three bits are exponents, and the rest 4 bits are mantissas (significands).
...
If the exponent is 7 then and mantissa is zero then the number is considered infinity (inf), otherwise it is not-a-number (nan).
...
class Float8():
    ...
    def calculate_value(self):
        ...
        if self.exponent == '111':
            ...
            for index, i in enumerate(self.mantissa):
                val = val + ((float(i)) * (2 ** (-index - 1)))
            val = val * (2 ** (-2))
            j = int(self.sign, 2)
            val = ((-1) ** j) * val
        
        else:
            k = int(self.exponent, 2)
            val = 1.0
            for index, i in enumerate(self.mantissa):
                val = val + ((float(i)) * (2 ** (-index - 1)))
            val = val * (2 ** (k - 3))
...
Over here, we will implement a 16 bit Floating point number system, where the first bit is a sign bit, next four bits are exponents, and the rest 11 bits are mantissas (significands).
